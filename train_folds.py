...
def get_training_and_dev_examples(config, test_fold=-1):
    ...
    for i, example in enumerate(examples):
        ...
        elif num_fold <= 1 or i % num_fold == dev_fold:
            dev_examples.append(example)
        else:
            train_examples.append(example)

    print("Find %d documents from %s use %d fro training and %d for dev" % (
    cnt, config['full_data_path'], len(train_examples), len(dev_examples)))
...
if __name__ == "__main__":
    ...
    if num_fold > 1:
        ...
        for test_fold in range(num_fold):
            ...
            # store experiment in the correspinding logs_old/ location
            ...
            with tf.Session() as session:
                ...
                train_tensorize_examples = [model.tensorize_example(example, is_training=True) for example in
                                            train_examples]
                ...
                while tf_global_step < config["training_steps"]:
                    ...
                    for example in train_tensorize_examples:
                        ...
                        tf_loss, tf_global_step, _ = session.run([model.loss, model.global_step, model.train_op],
                                                                 feed_dict=feed_dict)  # dont forget to feeeeed!
                        ...
                        if tf_global_step % report_frequency == 0:
                            ...
                            print(
                                "[{}] mention loss = {:.2f}, relation loss = {:.2f}, loss={:.2f}, steps/s={:.2f}".format(
                                    tf_global_step - original_step, average_mention_loss, average_relation_loss,
                        ...
                        if tf_global_step % eval_frequency == 0:
                            saver.save(session, os.path.join(log_dir, "model"), global_step=tf_global_step)
                            rel_output, coref_output, eval_summary_rel, eval_summary_coref, eval_f1 = model.evaluate(
                                session, dev_examples)
                            print(eval_summary_rel)
                            print('============')
                            print(eval_summary_coref)

                            if eval_f1 > max_f1:
                                max_f1 = eval_f1
                                util.copy_checkpoint(os.path.join(log_dir, "model-{}".format(tf_global_step)),
                                                     os.path.join(log_dir, "model.max.ckpt.data-00000-of-00001"))

                            writer.add_summary(util.make_summary(eval_summary_rel), tf_global_step)
                            writer.add_summary(util.make_summary(eval_summary_coref), tf_global_step)
            ...
            while tf_global_step < config["training_steps"]:
                ...
                for example in train_tensorize_examples:
                    ...
                    if tf_global_step % report_frequency == 0:
                        ...
                        average_relation_loss = accumulated_relation_loss / report_frequency
                        average_mention_loss = accumulated_mention_loss / report_frequency
                        print("[{}] mention loss = {:.2f}, relation loss = {:.2f}, loss={:.2f}, steps/s={:.2f}".format(
                            tf_global_step - original_step, average_mention_loss, average_relation_loss, average_loss,
                            steps_per_second))
                        writer.add_summary(util.make_summary({"loss": average_loss}), tf_global_step)
                        accumulated_loss = 0.0
                        accumulated_relation_loss = 0.0
                    ...
                    if tf_global_step % eval_frequency == 0:
                        saver.save(session, os.path.join(log_dir, "model"), global_step=tf_global_step)
                        rel_output, coref_output, eval_summary, eval_f1 = model.evaluate(session)

                        # if eval_f1 > max_f1:
                        #   max_f1 = eval_f1
                        #   util.copy_checkpoint(os.path.join(log_dir, "model-{}".format(tf_global_step)), os.path.join(log_dir, "model.max.ckpt.data-00000-of-00001"))
                        #
