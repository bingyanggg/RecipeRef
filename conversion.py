...
def process_ann(file_name):
    ...
    #     mentions_type = {}
    relations = {}
    relation_type = ["Coreference", "Transformed", "Ingredient_with-state-change_associated",
                     "Ingredient_without-state-change_associated"]
...
def constcut_jsonline_format(doc_id, lines, mentions_token, relations):
    ...
    for r_type in relations:
        ...
        for anphor_antecedent_pair in relations[r_type]:
            relations_token[r_type].append(
                [mentions_token[anphor_antecedent_pair[0]], mentions_token[anphor_antecedent_pair[1]]])
...
def convert_brat_to_training(annotation_path, token_conll_path, convert_path, convert_name):
    ...
    for doc_id in annotation_doc_id:
        temp_jsonlines_path = convert_path + doc_id + ".jsonlines"
